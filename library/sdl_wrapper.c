...
bool sdl_is_done(void *scene) {
  ...
  while (SDL_PollEvent(event)) {
    ...
    case SDL_KEYUP:
      ...
      double held_time = (timestamp - event_timestamp) / MS_PER_S;
...
void sdl_render_image(char *image_info_path, body_t *bod) {
  ...
  SDL_Surface *imageSurface = IMG_Load(image_info_path);
  if (imageSurface == NULL) {
    // Handle the case when loading the image failed
    printf("Failed to load image: %s\n", IMG_GetError());
    // You might want to return or exit the function here
  }
  printf("Heyyyy Bestieee");
